...
def plot_data(data,scores, files, env):
    ...
    fig = plt.figure()
    num_runs = 15
    colors = ['blue', 'red', 'green', 'purple', 'black', 'orange', 'lawngreen', 'saddlebrown', 'aqua', 'magenta']
    fig = plt.figure()
    ...
    fig.set_figwidth(11)
    fig.set_figheight(8)
    plt.savefig(f'{model_name}_comp_cost.png')


def get_files(file_type, state_reps, reward_function_types):
    files = []
    rootdir = '/Users/cmuslimani/Projects/Curriculum_MDP/Tabular/debug/four_rooms/new-reward-function/log-based/optimal/random-initalization/fixed-student-seed'
    batch_size = 64
    lr = 0.001
    alpha = 1.25
...
state_reps = ['policy_table',  'q_matrix','action_return'] #,
reward_function_types = [ 'cost'] #,'cost
files = get_files('teacher_return', state_reps,reward_function_types)
all_data = get_all_data(files)
print(all_data.keys())
scores = False
env = 'Four Rooms'
plot_data(all_data,scores, files, env)
